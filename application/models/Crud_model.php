...
class Crud_model extends CI_Model
{
    ...
    function file_view($type, $id, $width = '100', $height = '100', $thumb = 'no', $src = 'no', $multi = '', $multi_num = '', $ext = '.jpg')
    {
        ...
        else if ($multi == 'multi')
        {
            $num = $this
                ->crud_model
                ->get_type_name_by_id($type, $id, 'num_of_imgs');
            //$num = 2;
            $i = 0;
            $p = 0;
            $q = 0;
            ...
            else
            {
                ...
                {
                    return array(
                        base_url() . 'uploads/' . $type . '_image/default.jpg'
                    );
                }
                else
                {
    ...
    function product_link($product_id, $quick = '')
    {
        ...
        else
        {
            $name = url_title($this
                ->crud_model
                ->get_type_name_by_id('product', $product_id, 'title'));
    ...
    function customer_product_link($customer_product_id, $quick = '')
    {
        ...
        else
        {
            $name = url_title($this
                ->crud_model
                ->get_type_name_by_id('customer_product', $customer_product_id, 'title'));
    ...
    function blog_link($blog_id)
    {
        $name = url_title($this
            ->crud_model
            ->get_type_name_by_id('blog', $blog_id, 'title'));
    ...
    function vendor_link($vendor_id)
    {
        $name = url_title($this
            ->crud_model
            ->get_type_name_by_id('vendor', $vendor_id, 'display_name'));
            ...
            foreach ($all as $row):
                if ($type == 'add')
                {
                    ...
                    else
                    {
                        $return .= $this
                            ->crud_model
                            ->get_type_name_by_id($condition, $row, $c_match);
                    }
                    $return .= '</option>';
                }
                else if ($type == 'edit')
    ...
    function pagination($type, $per, $link, $f_o, $f_c, $other, $current, $seg = '3', $ord = 'desc')
    {
        ...
        return $this
            ->db
            ->get($type, $config['per_page'], $this
            ->uri
            ->segment($seg))->result_array();
    }
    ...
    {
        $additional_fields = $this
            ->crud_model
            ->get_type_name_by_id('product', $product_id, 'additional_fields');
    ...
    function get_customer_additional_fields($customer_product_id)
    {
        $additional_fields = $this
            ->crud_model
            ->get_type_name_by_id('customer_product', $customer_product_id, 'additional_fields');
        $ab = json_decode($additional_fields, true);
        $name = json_decode($ab['name']);
        $value = json_decode($ab['value']);
        $final = array();
    ...
    function decrease_quantity($product_id, $quantity, $sale_id = '')
    {
        $prev_quantity = $this
            ->crud_model
            ->get_type_name_by_id('product', $product_id, 'current_stock');
    ...
    function all_sale_date_n($product_id)
    {
        ...
        foreach ($sales as $i => $row)
        {
            if ($this
                ->session
                ->userdata('title') !== 'vendor' || $this->is_sale_of_vendor($row['sale_id'], $this
                ->session
                ->userdata('vendor_id')))
            {
                if ($this->product_in_sale($row['sale_id'], $product_id, 'id'))
    ...
    function month_total($type, $filter1 = '', $filter_val1 = '', $filter2 = '', $filter_val2 = '', $notmatch = '', $notmatch_val = '')
    {
        ...
        if ($type == 'sale')
        {
            ...
            foreach ($result as $row)
            {
                ...
                if ($this
                    ->session
                    ->userdata('title') == 'vendor')
                {
                    if ($this->sale_payment_status($row['sale_id'], 'vendor', $this
                        ->session
                        ->userdata('vendor_id')) == 'fully_paid')
                    {
                        //make version for vendor
                        $res_cat = $this
                            ->db
                        ...
                        foreach ($res_cat as $row1)
                        {
                            if ($p = $this->vendor_share_in_sale($row['sale_id'], $this
                                ->session
                                ->userdata('vendor_id') , 'paid'))
                            {
                                $p = $p['total'];
                                $a += $p;
                            }
        ...
        else if ($type == 'stock')
        {
                ...
                ->userdata('title') == 'admin')
            {
                $this
                    ->db
                    ->get_where('added_by', json_encode(array(
                    'type' => 'vendor',
                    'id' => $this
                        ->session
                        ->userdata('vendor_id')
                ...
                $this
                    ->db
                    ->get_where('datetime >= ', $ago);
                $this
                    ->db
                    ->get_where('datetime <= ', time());
                $result = $this
                    ->db
                    ->get('stock')
                    ->result_array();
    ...
    function email_invoice($sale_id)
    {
        ...
        if ($is_guest == "guest")
        {
            $info = json_decode($this
                ->db
                ->get_where('sale', array(
                'sale_id' => $sale_id
            ))->row()->shipping_address, true);
            $email = $info['email'];
        }
        else
        {
        ...
        $text = $this
            ->load
            ->view('front/shopping_cart/invoice_email', $page_data, true);
        $this
            ->email_model
            ->do_email($from, $from_name, $email, $sale_code, $text);
        $admins = $this
            ->db
        ...
        foreach ($admins as $row)
        {
            $this
                ->email_model
                ->do_email($from, $from_name, $row['email'], $sale_code, $text);
        }
    }
    ...
    function vendor_share_in_sale($sale_id, $vendor_id, $pay = '', $pay_type = '')
    {
        ...
        if ($this->sale_payment_status($sale_id, 'vendor', $vendor_id) == $pay || $pay == '')
        {
            if ($this
                ...
                ->get_where('sale', array(
                'sale_id' => $sale_id
            ))->row()->payment_type == $pay_type || $pay_type == '')
            {
    ...
    function vendor_share_total($vendor_id, $pay = '', $pay_type = '')
    {
        ...
        $commission = $this
            ->db
            ->get_where('business_settings', array(
    ...
    function rolelist($permission_roll = '')
    {
        $premission = json_decode($this
            ->db
            ->get_where('role_permission', array(
            'primary_roll' => $this
                ->session
                ->userdata('role')
        ))
            ->row()->secondary_roll, true);
?>
		
		<select name="role" id="cars" class="demo-chosen-select required">
        <?php $loop = $this
            ->db
            ->get_where('role')
            ->result_array();
        foreach ($loop as $row)
        {
            if (1 != $this
                ...
                ->userdata('role'))
            {
                if (in_array($row['role_id'], $premission))
                {
?>
      <option value="<?php echo $row['role_id'] ?>" <?php if ($permission_roll == $row['role_id'])
                    { ?>  selected <?php
...
?>
		...
		<select name="status" onChange="changestatus(this.value)" id="status" class="demo-chosen-select required">
        
      <option value="Accepted" <?php if ($status == 'Accepted')
        { ?>  selected <?php
        } ?>>Accepted</option> 
       <option value="Packed" <?php if ($status == 'Packed')
        { ?>  selected <?php
        } ?>>Packed</option>
       <option value="Shipped" <?php if ($status == 'Shipped')
        { ?>  selected <?php
        } ?>>Shipped</option>
    ...
    {
        $payment_status = json_decode($this
            ->db
            ->get_where('sale', array(
            'sale_id' => $sale_id
    ...
    function sub_details_by_cat($cat = '')
    {
        ...
        foreach ($subs as $row)
        {
            $result[] = array(
                ...
                'min' => round($this
                    ->crud_model
                    ->get_range_lvl('sub_category', $row['sub_category_id'], "min")) ,
                'max' => round($this
                    ->crud_model
                    ->get_range_lvl('sub_category', $row['sub_category_id'], "max")) ,
    ...
    function vendor_products_by_cat($vendor, $category)
    {
        $products = $this
            ->db
            ->from('product')
            ->where('added_by', '{"type":"vendor","id":"' . $vendor . '"}')->where('category', $category)->where('status', 'ok')
            ->get()
            ->result_array();
        return $products;
    }
    function get_vendor_brands($vendor)
    {
        $brands = $this
            ...
            ->select('brand')
            ->from('product')
            ->where('added_by', '{"type":"vendor","id":"' . $vendor . '"}')->where('status', 'ok')
            ->get()
            ->result_array();
        return $brands;
    }
    ...
    function can_add_product($vendor)
    {
        ...
        if ($membership == '0')
        {
            $max = $this
                ->db
                ->get_where('general_settings', array(
                'type' => 'default_member_product_limit'
            ))
                ->row()->value;
        }
        else
    ...
    function is_publishable($product_id)
    {
        ...
        $physical_product_activation = $this
            ->db
            ->get_where('general_settings', array(
            'type' => 'physical_product_activation'
        ))
            ->row()->value;
        $digital_product_activation = $this
            ->db
            ->get_where('general_settings', array(
            'type' => 'digital_product_activation'
        ))
            ->row()->value;
    ...
    function is_publishable_count($type, $id, $vendor_id = '')
    {
        ...
        if ($vendor_id !== '')
        {
            $this
                ->db
                ->where('added_by', json_encode(array(
                'type' => 'vendor',
                'id' => $vendor_id
            )));
        }
        ...
        foreach ($vendors as $row)
        {
            if ($row['membership'] !== '0')
            {
                if ($row['member_expire_timestamp'] < time())
                {
                    ...
                    $this
                        ->email_model
                        ->membership_upgrade_email($row['vendor_id']);
                    $this
                        ->email_model
                        ->membership_upgrade_email_to_admin($row['vendor_id']);
                }
            }
        }
    }
    ...
    function upgrade_membership($vendor, $membership)
    {
        ...
        $this
            ->email_model
            ->membership_upgrade_email($vendor);
        $this
            ->email_model
            ->membership_upgrade_email_to_admin($row['vendor_id']);
    }
    ...
    function admin_permission($codename)
    {
        ...
        {
            $role = $admin->role;
            $role_permissions = json_decode($this
                ->crud_model
                ->get_type_name_by_id('role', $role, 'permission'));
    ...
    function ip_data()
    {
            ...
            ->is_ajax_request())
        {
            $this
                ->session
                ->set_userdata('timestamp', time());
            $user_data = $this
                ->session
                ->userdata('surfer_info');
            ...
            if (!$user_data)
            {
                if ($_SERVER['HTTP_HOST'] !== 'localhost')
                {
                    ...
                    $this
                        ->session
                        ->set_userdata('surfer_info', $ip_data);
                }
            }
        }
    }
    ...
    function add_compare($product_id)
    {
        if ($this
            ->session
            ->userdata('compare') == '' || $this
            ->session
            ->userdata('compare') == 'null')
        {
            $this
                ->session
                ->set_userdata('compare', '[]');
        }
        $compared = json_decode($this
            ->session
            ->userdata('compare') , true);
    ...
    function get_range_lvl($by = "", $id = "", $type = "")
    {
        $physical = $this
            ->crud_model
            ->get_settings_value('general_settings', 'physical_product_activation');
        $digital = $this
            ->crud_model
            ->get_settings_value('general_settings', 'digital_product_activation');
        $vendor = $this
            ->crud_model
            ->get_settings_value('general_settings', 'vendor_system');
        if ($type == "min")
        {
            $set = 'asc';
        }
    ...
    function get_range_lvl_cproduct($by = "", $id = "", $type = "")
    {
        ...
        if (count($a = $this
            ->db
            ->get_where('customer_product', array(
            $by => $id,
            'status' => 'ok'
        ))->result_array()) > 0)
        {
    ...
    function can_download($product)
    {
        ...
        if ($this
            ...
            ->userdata('vendor_login') == 'yes')
        {
            if ($this->is_added_by('product', $product, $this
                ->session
                ->userdata('vendor_id') , 'vendor'))
            {
                return true;
            }
            else
    ...
    function ticket_unread_messages($ticket_id, $user_type)
    {
        ...
        if ($ticket_id !== 'all')
        {
            $msgs = $this
                ->db
                ->get_where('ticket_message', array(
                'ticket_id' => $ticket_id
            ))->result_array();
        }
    ...
    function ticket_message_viewed($ticket_id, $user_type)
    {
        ...
        foreach ($msgs as $row)
        {
            ...
            $this
                ->db
                ->where('ticket_message_id', $row['ticket_message_id']);
            $this
                ->db
                ->update('ticket_message', array(
    ...
    function category_link($cat = '', $scat = '', $brand = '')
    {
        ...
        if ($cat !== '')
        {
            $cat_name = $this
                ->crud_model
                ->get_type_name_by_id('category', $cat, 'category_name');
        }
        if ($scat !== '')
        {
            $scat_name = $this
                ->crud_model
                ->get_type_name_by_id('sub_category', $scat, 'sub_category_name');
        }
        if ($brand !== '')
        {
            $brand_name = $this
                ->crud_model
                ->get_type_name_by_id('brand', $brand, 'name');
        }
    ...
    function product_list_set($speciality, $limit, $id = '')
    {
        ...
        $digital_product_activation = $this
            ->crud_model
            ->get_settings_value('general_settings', 'digital_product_activation');
        $vendor_system_activation = $this
            ->crud_model
            ->get_settings_value('general_settings', 'vendor_system');

        if ($vendor_system_activation == 'ok')
        {
            $approved_vendors = $this
                ->db
                ->get_where('vendor', array(
                'status' => 'approved'
            ))
    ...
    */
    ...
    function set_category_data($cat_id)
    {
        ...
        $this
            ->db
            ->where('type', 'data_all_brands');
        $this
            ->db
            ->update('general_settings', $data1);

        $data2['value'] = join(';;;;;;', $this->get_vendors_by('category', '0'));
        $this
            ->db
            ->where('type', 'data_all_vendors');
        $this
            ->db
            ->update('general_settings', $data2);
    }

    function is_product_bought($buyer_id, $product_id)
    {
        ...
        $this
            ->db
            ->like('product_details', '"id":"' . $product_id . '"');
        $this
            ->db
            ->like('payment_status', '"status":"paid"');
        $this
            ->db
            ->limit(1);
    ...
    function calculate_sum($table = '', $coloum1 = '', $para1 = '', $select)
    {
    ...
    function calculate_totalsales($table = '', $select = '')
    {
    ...
    function count_bargraph($mon, $type)
    {
        $year = date('Y');
        if ($mon == '1')
        {
